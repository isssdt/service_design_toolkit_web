...
public class ConstantValues {
    ...
    /**
     * SELECT s FROM SdtUser s WHERE s.username = :username and s.password =
     * :password
     */
    ...
    /**
     * SELECT t FROM TouchPoint t WHERE t.journeyId.journeyName = :journeyName
    ...
    /**
     * SELECT t FROM TouchpointFieldResearcher t WHERE
     * t.fieldResearcherId.sdtUser.username = :username and
     * t.touchpointId.journeyId IN (SELECT J.journeyId FROM
     * JourneyFieldResearcher J WHERE J.fieldResearcherId.sdtUser.username =
     * :username and J.status = 'IN PROGRESS')
     */
    ...
    /**
     * SELECT COUNT(1) FROM TouchpointFieldResearcher t WHERE
     * t.touchpointId.journeyId.journeyName = :journeyName AND t.ratingId.id = 3
     */
    public static final String QUERY_GET_COUNT_NEUTRAL_RATING_FOR_TOUCH_POINT_OF_JOURNEY = "TouchpointFieldResearcher.00002";

    /**
     * SELECT COUNT(1) FROM TouchpointFieldResearcher t WHERE
     * t.touchpointId.journeyId.journeyName = :journeyName AND t.ratingId.id in
     * (1,2)
     */
    ...
    /**
     * SELECT j FROM JourneyFieldResearcher j WHERE
     * j.fieldResearcherId.sdtUser.username = :username
     */
    ...
    /**
     * SELECT j FROM Journey j WHERE j.startDate <= :startDate and j.endDate >=
     * :endDate and j.canBeRegistered = 'Y'
     */
